...
def safe_log(data: Tensor):
    return data.clamp(min=CONST_log_min).log()  # 计算clamp处理后的张量的自然对数
...
def pe_encode(sequence_length: int, embedding_size: int) -> Tensor:
    ...
    pe = torch.zeros((sequence_length, embedding_size))  # [sequence_length, embedding_size]
    pos = torch.arange(sequence_length).unsqueeze(1) # [sequence_length,1]
    pe[:, 0::2] = torch.sin(
...
def pe_encode_float(x: Tensor, max_freq: float, embedding_size: int) -> Tensor:
    pe = torch.zeros(list(x.shape) + [embedding_size], device=x.device) # (N, M)->(N, M ,E)
