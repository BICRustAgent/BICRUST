...
# MixtureSameFamily：同族混合分布
class GMM(MixtureSameFamily):
    def __init__(self, mix_wt_logits, means, std_devs):
        mix_wts = torch_Categorical(logits=mix_wt_logits, validate_args=False) #定义混合权重
        components = Normal(means, std_devs, validate_args=False)  # 定义正态分布(可能是多个正态分布)
        super().__init__(mix_wts, components, validate_args=False)  # 创建混合分布
...
class Categorical(DiscreteDistribution):
    def __init__(self, logits):
        self.categorical = torch_Categorical(logits=logits, validate_args=False) # validate_arg创建时验证参数的有效性
...
class GMMFactory(CtsDistributionFactory):
    ...
    def get_dist(self, params, input_params=None, t=None):
        # 沿着最后一个维度将params分隔为三部分
        mix_wt_logits, means, std_devs = params.chunk(3, -1) # 如果在该维度上的元素无法被3整除，那么分割得到的最后一个块可能会包含较少的元素
        if self.log_dev:
            std_devs = safe_exp(std_devs)
        std_devs = std_devs.clamp(min=self.min_std_dev, max=self.max_std_dev)
        return GMM(mix_wt_logits, means, std_devs) # mix_wt_logits是混合权重，创建(means, std_devs)正态分布族
...
class DeltaFactory(CtsDistributionFactory):
    ...
    def get_dist(self, params, input_params=None, t=None):
        ...
        # 将params.squeeze(-1)每个元素裁剪到[-self.clip_range, self.clip_range],然后输出
        # 将参数params每个元素的取值范围限制到[-self.clip_range,self.clip_range]
...
def noise_pred_params_to_data_pred_params(noise_pred_params: torch.Tensor, input_mean: torch.Tensor, t: torch.Tensor, min_variance: float, min_t=1e-6):
    ...
    noise_pred_params：epsilon噪音
    data_pred_params: parameters that predict the output data
    """
    data_shape = list(noise_pred_params.shape)[:-1]
    noise_pred_params = sandwich(noise_pred_params) # 第一维和最后一维的大小不变
    input_mean = input_mean.flatten(start_dim=1) # 第0维（通常是批次维度）保持不变，从第1维开始的所有维度将被展平
    if torch.is_tensor(t): # 如果t是个张量展凭，如果t是个数值，广播成一个和input_mean相同形状的张量
        t = t.flatten(start_dim=1)
    else:
        t = (input_mean * 0) + t
    alpha_mask = (t < min_t).unsqueeze(-1) # 生成了一个布尔型张量，并在该张量最后一个维度后增加一个新的维度
    posterior_var = torch.pow(min_variance, t.clamp(min=min_t)) #
    gamma = 1 - posterior_var  # gamma（Γ）
    ...
    if noise_pred_params.size(-1) == 1:
        ...
        data_pred_params.append(mix_wt_logits)  ### mix_wt_logits

    data_pred_mean = A - (B * noise_pred_mean)  # data_pred_mean ~ output distribution
    data_pred_mean = torch.where(alpha_mask, 0 * data_pred_mean, data_pred_mean) # 将张量data_pred_mean，小于min_t的元素置零，其他元素保持不变
    data_pred_params.append(data_pred_mean) ### data_pred_mean
    ...
    if noise_pred_params.size(-1) >= 2:
        ...
        data_pred_params.append(data_pred_dev)  ### data_pred_dev
    data_pred_params = torch.cat(data_pred_params, -1)
    data_pred_params = data_pred_params.reshape(data_shape + [-1]) # data_pred_params前几个维度和data_shape一致，最后一个维度自动计算
...
class PredDistToDataDistFactory(DiscreteDistributionFactory):
    ...
    # 创建一个Output Data Distribution
    def get_dist(self, params, input_params, t):
        """
        params：output parameters that predict the noise
        input_params[0]：mean of input distribution
        """
