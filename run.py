...
parser.add_argument('--data', default="Triangle", type=str, choices=('cifar10', 'cifar100', 'pathfinder', 'MNIST', 'Triangle'), help='data to train on')
parser.add_argument('--version', default=0, type=int, help='version for shared transformer-- 0 or 1')
parser.add_argument('--num_layers', default=1, type=int, help='num of layers') ######
parser.add_argument('--num_templates', default=1, type=int, help='num of templates for shared transformer')
parser.add_argument('--num_heads', default=4, type=int, help='num of heads in Multi Head attention layer')
parser.add_argument('--batch_size', default=2, type=int, help='batch_size to use') #batch_size
...
parser.add_argument('--h_dim', type=int, default=64)  # --h_dim
parser.add_argument('--ffn_dim', type=int, default=512)  # --ffn_dim
parser.add_argument('--num_gru_schemas', type=int, default=2)
parser.add_argument('--num_attention_schemas', type=int, default=2)
parser.add_argument('--schema_specific', type=str2bool, default=True)
parser.add_argument('--num_eval_layers', type=int, default=6)
parser.add_argument('--share_vanilla_parameters', type=str2bool, default=True)       # --share_vanilla_parameters
parser.add_argument('--num_digits_for_mnist', type=int, default=3)
parser.add_argument('--use_topk', type=str2bool, default=True)
parser.add_argument('--topk', type=int, default=20)
parser.add_argument('--shared_memory_attention', type=str2bool, default=True)        # shared_memory_attention
parser.add_argument('--mem_slots', type=int, default=8)
...
# #
# #     trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, num_workers=0, shuffle=False)
# #     testloader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, num_workers=0, shuffle=False)
# #     num_classes = 10
# #     image_size = 100
# #     channels = 1
...
class model(nn.Module):
    ...
    def forward(self, img, mask=None):
        ...
        # print(x.size()) torch.Size([64, 64, 48])
        # print(x.type())
        x = self.patch_to_embedding(x)  # torch.Size([64, 64, 256])
        # print(x.size()) torch.Size([64, 64, 256])

        b, n, _ = x.shape
        # print(x.shape)

        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b=b)  # [1,1,args.h_dim]->[b,1,args.h_dim]
        x = torch.cat((cls_tokens, x), dim=1)
        # print(x.size()) torch.Size([64, 65, 256])

        x = self.net(x)
        # print("output of the net  "+ str(x.shape))  torch.Size([64, 65, 256])
...
def mnist_acc(outputs, targets):
    ...
    equality = torch.eq(outputs, targets)  # boolean
    equality = equality.int()              # boolean->0/1
