...
class SelectAttention(nn.Module):
    ...
    def __init__(self, d_read, d_write, d_k=16, num_read=5, num_write=5, share_query=False, share_key=False):
        ...
        if not share_key:
            self.gll_write = GroupLinearLayer(d_write, d_k, num_write)  # d_write, d_k = 16，num_write = 5
        else:
            self.gll_write = nn.Linear(d_write, d_k)  # d_k=16
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        args.encoder_embed_dim = embed_dim  # mem_size
        args.encoder_ffn_embed_dim = ffn_dim
        args.encoder_attention_heads = num_heads
        args.dropout = dropout
        args.shared_memory_attention = shared_memory_attention  # shared_memory_attention = False
        args.num_steps = num_steps
        args.null_attention = null_attention
        args.regressive = regressive  # default regressive = False

        self.num_layers = num_layers
        self.shared_memory_attention = shared_memory_attention  # default shared_memory_attention = False,
        self.shared_memory_percentage = shared_memory_percentage  # default  shared_memory_percentage = 0.1,
        ...
        if not self.functional:
            ...
            args.self_attention = True
            self.inp_att = nn.ModuleList(
                [TransformerEncoderLayerVanilla(args=args) for _ in range(num_attention_schemas)])
            ...
            # self.gru_pool是一个包含num_gru_schemas个nn.GRUCell模块的列表。
            # 我们可以通过索引来访问其中的每个模块，例如self.gru_pool[0]表示第一个nn.GRUCell模块
            ...
            self.select_attention = SelectAttention(hidden_dim + hidden_dim, hidden_dim, num_read=1,
                                                    num_write=num_gru_schemas)

        self.pe = PositionEncoder(args.encoder_embed_dim)  # [max_seq_len = 300, args.encoder_embed_dim]
    ...
    def forward(self, x, mask=None, num_layers=None):
        ...
        x = self.pe(x)  # self.pe = PositionEncoder(args.encoder_embed_dim)
        ...
        if not self.functional:
            ...
            if self.shared_memory_attention:
                for inp_att in self.inp_att:  # num_attention_schemas
            ...
            for i in range(0, num_layers):
                gru_ins = []
                # num_attention_schemas
                # self.inp_att=nn.ModuleList([TransformerEncoderLayerVanilla(args=args) for _
                # in range(num_attention_schemas)])
                ...
                # ###############################################
                x = x.permute(1, 0, 2)
                x = x.reshape(B * T, -1).unsqueeze(1)  # (B*T, 1, D)
...
#                              schema_specific=False, shared_memory_attention=True, mem_slots=8, num_steps=20).cuda()
#     t1 = time.time()
#     for i in range(5):
#         x = TE1(x)
#     print(time.time() - t1)
...
#                              schema_specific=True, shared_memory_attention=False, mem_slots=8, num_steps=20).cuda()
#     t1 = time.time()
#     for i in range(5):
#         x = TE2(x)
#     print(time.time() - t1)
