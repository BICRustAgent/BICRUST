...
from bayesianflow_utilities.utils_model import sandwich, pe_encode, pe_encode_float
...
class TextInputAdapter(nn.Module):
    ...
    def __init__(
        ...
        if learn_pos_embedding:
            self.pos_embedding = nn.Embedding(seq_len, output_size) # use torch.nn.init.normal to initate a tensor ,shaped [seq_len, output_size]
        else:
            self.register_buffer("pos_embedding", pe_encode(seq_len, output_size))  # pos_embedding is persistent
    ...
    def forward(self, probs: torch.Tensor, t: torch.Tensor) -> Tensor:
        """
        1 -> what is the meaning of probs
         probs输入维度应该是3维，t应该是2维
        inp_emb : [batch_size, seq_len, output_size]
         pos_emb ： [ batch_size, seq_len, output_size]
         t_emb : [batch, output_size, 1]
        """
        inp_emb = self.inp_embedding(2 * probs - 1)  # vocab_size -> output_size
        if self.learn_pos_embedding:
            # self.pos_embedding = nn.Embedding(seq_len, output_size)
            # 以 torch.arange(0, probs.size(1)) 为索引进行查表操作
...
class FourierImageInputAdapter(nn.Module):
    ...
    def __init__(
        ...
        if learn_pos_feats:
            ...
            x = torch.linspace(-1.0, 1.0, steps=input_shape[0])  # size of the constructed tensor
            y = torch.linspace(-1.0, 1.0, steps=input_shape[1])
            # x_pos, y_pos = torch.meshgrid(x, y, indexing="ij")  # (input_shape[0],input_shape[1])
            x_pos, y_pos = torch.meshgrid(x, y)  # (input_shape[0],input_shape[1])

            pos = torch.stack((x_pos, y_pos), dim=-1)  # (input_shape[1],input_shape[0],2)
            pos = pos.reshape(-1, 2)  # (input_shape[0]*input_shape[1], 2)
            ...
            bands = torch.stack((x_bands, y_bands), dim=0)  # (2,n_freq_bands)

            # 使用None增加一个新的维度
            vals = pos[:, :, None] * bands[None, :, :]   # (input_shape[0]*input_shape[1], 2,1) *(1, 2,n_freq_bands)
    ...
    def forward(self, img: Tensor, t: Tensor) -> Tensor:
        ...
        if self.add_mask:
            all_feat_list.append(t_feats)    # t_feats
        if self.add_pos_feats:
            all_feat_list.append(fourier_feats)  # fourier_feats
...
class OutputAdapter(nn.Module):
    def __init__(self, input_height: int, output_channels: int, output_height: int, output_width: int):
        super().__init__()
        self.output_channels = output_channels
        self.output_height = output_height
        self.output_width = output_width
        self.output_projection = nn.Linear(
            input_height, output_channels
        )

    def forward(self, inp: torch.Tensor) -> torch.Tensor:
        output = self.output_projection(inp)
        return output.reshape(
            output.size(0),  self.output_height, self.output_width,  self.output_channels
        )  # (output.size(0), output_width ? ,output_channels,output_height)
