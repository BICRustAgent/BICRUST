...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
            ...
            add_bias_kv=False,
            add_zero_attn=False,
            self_attention=False, # self attention
            encoder_decoder_attention=False,  #encoder attention or decoder attention
            q_noise=0.0,
            qn_block_size=8,
            nblocks=1,
        ...
        self.self_attention = self_attention  # default self_attention=False
        ...
        if not self.shared_memory_attention:
            ...
            self.q_proj = quant_noise(GroupLinearLayer(embed_dim // nblocks, embed_dim // nblocks, nblocks, bias=bias),
                                      q_noise, qn_block_size)
            self.out_proj = quant_noise(GroupLinearLayer(embed_dim // nblocks, embed_dim // nblocks, nblocks, bias=bias),
                                      q_noise, qn_block_size)
            if add_bias_kv:
            ...
            self.add_zero_attn = add_zero_attn  # default add_zero_attn=False
        ...
        if self.shared_memory_attention:
            ...
            print('Null attention:' + str(null_attention))  # default null_attention = False
            ...
            if not regressive:
                print("+++ relational memory volatile +++")
    ...
    def prepare_for_tpu_(self, **kwargs):  # tpu：=Tensor Processing Unit
        self.tpu = True
    ...
    def forward(
            self,
            query,
            key: Optional[Tensor],  # Tensor Or None
            value: Optional[Tensor],
            key_padding_mask: Optional[Tensor] = None,
            incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
            need_weights: bool = True,
            static_kv: bool = False,
            attn_mask: Optional[Tensor] = None,  # Tensor Or None default:=None
        ...
        if (
            ...
            if self.shared_memory_attention:
                memory, _ = F.multi_head_attention_forward(
                    ...
                    self.num_heads,
                    torch.empty([0]),
                    torch.cat((self.q_proj_memory.bias, self.k_proj.bias, self.v_proj.bias)),  # in_proj_bias
                    self.bias_k,  # bias_k
                    self.bias_v,  # bias_v
                    self.add_zero_attn,  # add a new batch of zeros to the key and value sequences at dim=1
                    self.dropout_module.p,  # dropout.p
                    self.out_proj_memory.weight,  # out_proj_weight
                    self.out_proj_memory.bias,  # out_proj_bias
                    self.training or self.dropout_module.apply_during_inference,  # apply dropout if it is TRUE
                    key_padding_mask,
                    need_weights,
                    attn_mask,
        ...
        if not self.shared_memory_attention:
            ...
            # 3.2 处理bias和attn_mask
            if self.bias_k is not None:
                assert self.bias_v is not None
                k = torch.cat([k, self.bias_k.repeat(1, bsz, 1)])  # 没有指定维度，则默认dim=0
            ...
            if self.add_zero_attn:
                ...
                v = torch.cat([v, v.new_zeros((v.size(0), 1) + v.size()[2:])], dim=1)  # dim0=k.size(0), dim1=1
                if attn_mask is not None:
                    attn_mask = torch.cat(
                        [attn_mask, attn_mask.new_zeros(attn_mask.size(0), 1)], dim=1
                    )
            ...
            attn_weights = MultiheadAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz)  # return attn_weights
        ...
        # 4 使用共享内存
            ...
            if self.long_term_memory is None:
                self.long_term_memory = self.relational_memory.initial_long_term_memory(query.size(1)).to(query.device)
                # (batch_size, self.mem_slots, self.mem_size, self.mem_size)

            key = key.transpose(1, 0)

            _, self.memory, self.long_term_memory, out_hx_mem_new = self.relational_memory(
                inputs=key,
