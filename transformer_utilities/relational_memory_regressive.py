...
from bayesianflow_utilities.bfn_utils import sample_t, update_input_params, make_from_cfg, make_config
...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, mem_slots, head_size, input_size, output_size, num_heads=1, num_blocks=1, forget_bias=1.,
                 ...
                 topk=3, num_steps=5, long_term_memory_c=5, alpha=1, config_name= 'mnist_discrete.yaml', num_time_steps=200,
                 null_attention=False):
        super(RelationalMemory, self).__init__()


        # ############## params for BFN  ############## #
        self.long_term_memory_c = long_term_memory_c
        ...
        self.output_projector = nn.Linear(self.output_size, self.input_size)  # self.output_size=memory_slots*memory_size
        ...
        self.return_all_outputs = return_all_outputs  # default return_all_outputs=False

        self.null_attention = null_attention  # default null_attention = False
    ...
    def initial_state(self, batch_size, ts, trainable=False):
        ...
        #  (batch_size, self.mem_slots, self.mem_slots) 单位矩阵
        ...
        init_state = init_state.unsqueeze(1)  # (batch_size, 1, self.mem_slots, self.mem_size)
        init_state = init_state.repeat(1, ts, 1, 1)  # (batch_size, ts, self.mem_slots, self.mem_size)
        init_state = init_state.reshape(batch_size * ts, self.mem_slots,
                                        -1)  # (batch_size * ts, self.mem_slots, self.mem_size)
    ...
    def initial_long_term_memory(self, batch_size):
        ...
        if True:
            ...
            init_state = torch.unsqueeze(init_state, dim=-1)
            init_state.repeat(1, 1, 1, self.long_term_memory_c)


        else:
            init_state = torch.randn(batch_size, self.mem_slots, self.mem_size, self.long_term_memory_c)
    ...
    def multihead_attention(self, input, memory, mask=None):
        ...
        q = self.query_proj(memory)  # self.mem_size->self.key_size*self.num_heads
        k = self.key_proj(input)     # self.mem_size->self.key_size*self.num_heads
        v = self.value_proj(input)  # self.mem_size->self.key_size*self.num_heads

        # (B,T,D)->(B,T,self.num_heads,self.num_heads/D)->(B, self.num_heads, T, self.num_heads/D)
        ...
        self.score_log = scores  # the log of scores
        if True:
            if self.use_topk:
                topk = torch.topk(scores, dim=-1, k=self.topk)  # (B, self.num_heads, T, self.num_heads/D)
    ...
    def create_gates(self, inputs, memory):
        ...
        if len(inputs.shape) == 3:  # （B，T, D）
            ...
            inputs = inputs.view(inputs.shape[0], -1)  # (B,T*D)
            # matmul for equation 4 and 5
            # there is no output gate, so equation 6 is not implemented
            # print(inputs.size())
            # #self.input_gate_projector = nn.Linear(self.mem_size, self.num_gates)
            gate_inputs = self.input_gate_projector(inputs)  # (B,self.num_gates)
            gate_inputs = gate_inputs.unsqueeze(dim=1)  # (B,1,self.num_gates)
            gate_memory = self.memory_gate_projector(
                memory)  # (batch_size, self.mem_slots, self.mem_size)->(batch_size, self.mem_slots, self.gate_size)
        else:
        ...
        # (batch_size, self.mem_slots, self.gate_size/2)
    ...
    def attend_over_memory(self, inputs, memory, mask=None):
        ...
        Perform multiheaded attention over `memory`.
            Args:
              ...
              self.num_blocks: num_blocks: Number of times to compute attention per time step. Defaults to 1.
            Returns:
              The attended-over memory.
        """
        for _ in range(self.num_blocks):
            attended_memory = self.multihead_attention(inputs, memory, mask=mask)  # attended_memory.shape==memort.shape
            # key: inputs
            # value: inputs
            # query: memory
    ...
    def forward_step(self, inputs, memory, long_term_memory, treat_input_as_matrix=False, mask=None, other_inp=None):
        """
        ...
        if self.gate_style == 'unit' or self.gate_style == 'memory':
            ...
            input_gate, forget_gate = self.create_gates(other_inp.unsqueeze(1), memory)  ############################3
            # equation 7 calculation
            next_memory = input_gate * torch.tanh(next_memory)
            next_memory += forget_gate * memory  # Mt:=input_gate * tanh(M) + forget_gate * Mt-1

        output = next_memory.view(next_memory.shape[0], -1)
        return output, next_memory  # next_memory.size:= (batch_size, self.mem_slots, self.mem_size)

    def forward(self, inputs, memory, long_term_memory):
        ...
        mask = torch.tril(mask)  # 将 mask 张量转换为一个下三角矩阵，其中主对角线及其以上的元素都被置为零
        mask = mask.unsqueeze(0)  # (1,T,T)
        mask = mask.repeat(inputs.size(0), 1, 1)  # (B,T,T)
        mask = mask.reshape(mask.size(0) * mask.size(1), -1)  # (B*T,T)

        inputs_ = inputs.unsqueeze(2)  # (B,T,1,D)
        inputs_ = inputs_.repeat(1, 1, inputs.size(1), 1)  # (B,T,T,D)
        inputs_ = inputs_.reshape(B * T, T, -1)  # (B*T,T,D)
        ...
        # else:
        # output, memory
        # memory.size:=(batch_size, self.mem_slots, self.mem_size)
        logits, memory = self.forward_step(inputs_, memory, treat_input_as_matrix=True, mask=mask,
                                           other_inp=inputs.reshape(B * T, -1))

        # self.output_projector = nn.Linear(self.output_size, self.input_size)
        # def output_size(self):
        #     return self.mem_slots * self.mem_size
        # memory.view(memory.shape[0], -1)->(batch_size, self.mem_slots*self.mem_size)
        # (batch_size, self.mem_slots*self.mem_size)->(batch_size,input_size)
...
# seq_length = 1
# batch_size = 32
# model = RelationalMemory(mem_slots=10, head_size=64, input_size=input_size, output_size=0, num_heads=8, num_blocks=1, forget_bias=1., input_bias=0.)
# model_memory = model.initial_state(batch_size=batch_size, ts=1)  # [batch_size = 32,mem_slots=10, mem_size=512]
#
# # random input
