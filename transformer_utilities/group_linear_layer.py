...
class GroupLinearLayer(nn.Module):
    ...
    def forward(self,x):
	 ...
	 #blocks, ts*bs, nhid
        ...
        x = x.permute(1,0,2) #self.nb==num_blocks, ts*bs, m//self.nb
        x = torch.bmm(x,self.weight) # self.weight.shape= (num_blocks, din, dout)  x.shape=(number_block,ts*bs,din)
        x = x.permute(1,0,2)
        
        if not self.bias is None:
            x = x + self.bias  # bias.shape=(num_blocks,dout)
