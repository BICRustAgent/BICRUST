...
class RelationalMemory(nn.Module):
    ...
    Args:
      ...
      head_size: The size of an attention head.  #不确定查询结果：注意力头的维度等于输入序列的维度除以注意力头的数量
    ...
    def initial_state(self, batch_size, trainable=False):
        ...
        Returns:
          init_state: A truncated or padded matrix of size
            (batch_size, self.mem_slots, self.mem_size).
        """
        # (batch_size, self.mem_slots, self.mem_slots)
    ...
    def multihead_attention(self, input, memory):
        ...
        q = self.query_proj(memory) # self.query_proj = nn.Linear(self.mem_size, self.key_size * self.num_heads)
        k = self.key_proj(input)    # self.key_proj = nn.Linear(self.mem_size, self.key_size * self.num_heads)
        v = self.value_proj(input)  # self.value_proj = nn.Linear(self.mem_size, self.value_size * self.num_heads)
        ...
        v = v.reshape(v.size(0), v.size(1), self.num_heads, -1).permute(0, 2, 1, 3) # (batch_size, num_heads, seq_len, seq_len)
        ...
        if not self.null_attention:
            if self.use_topk:
                ...
                mask = torch.zeros(scores.size()).to(scores.device) #计算掩码
                mask.scatter_(3, topk.indices, 1)                   #计算掩码
                scores = scores * mask
        else:
    ...
    def create_gates(self, inputs, memory):
        ...
        # if inputs.shape==(d1, d2, d3) len(input.shape)==3
        if len(inputs.shape) == 3:
            ...
            gate_inputs = gate_inputs.unsqueeze(dim=1)  # self.input_gate_projector = nn.Linear(self.mem_size * num_steps, self.num_gates)
            gate_memory = self.memory_gate_projector(memory)  # self.memory_gate_projector = nn.Linear(self.mem_size, self.num_gates)
        else:
        ...
        assert input_gate.shape[2] == forget_gate.shape[2] # input_gate 和 forget_gate 的最后一个维度大小相同
